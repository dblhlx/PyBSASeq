...
The default p-value for identifying ltaSNPs from the SNP data set is 0.01 (alpha), the default p-value for identifying ltaSNPs from the simulation sample is 0.1 (smalpha), and the default size of the simulation sample is 300 (smplsize). These values can be changed using the following options:
...
#### Workflow
1. SNP filtering
2. Perform Fisher's exact test using the AD values of each SNP from both bulks. A p-value will be otained. In the meantime, simulated REF/ALT reads of each SNP is obtained under null hypothesis via simulation, and again Fisher's exact test is performed using the simulated AD values  
